...
# the main part of `/root/easy-xray-main/ex.sh conf` command, generates config files for server and clients
conf () {
    export PATH=$PATH:/usr/local/bin/ # brings xray to the path for sudo user
    check_command xray "needed for config generation" "to install xray, try: sudo /root/easy-xray-main/ex.sh install"
...
(9) your variant"
    read number
    default_fake_site="www.microsoft.com"
    if [ -v $number ]
    then
    ...
    then
        ...
        cat /root/easy-xray-main/template_site4cdn.conf \
            | sed "s!server_domain_name!${server_name4cdn}!" \
            | sed "s!www.youtube.com!${fake_site}!" \
            | sed "s!your_service_name!${service_name}!" \
            > /root/easy-xray-main/conf/site4cdn.conf
        cp /root/easy-xray-main/conf/site4cdn.conf /etc/nginx/sites-enabled/
    fi
    #
    ## Make server config ##
    jsonc2json template_config_server.jsonc \
        ...
        > /root/easy-xray-main/conf/config_server.json
    # make the user (not root) the owner of the file
    [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_server.json
    vnext=" [
            {
                \"address\": \"${address}\",
                \"port\": 443,
    ...
    jsonc2json template_config_client.jsonc \
        ...
        > /root/easy-xray-main/conf/config_client.json
    # make the user (not root) an owner of a file
    [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_client.json
    if [ -f "/root/easy-xray-main/conf/config_client.json" ] && [ -f "/root/easy-xray-main/conf/config_server.json" ]
    then
    ...
    then
        jsonc2json template_config_client_cdn.jsonc \
            ...
            > /root/easy-xray-main/conf/config_client_cdn.json
        # make the user (not root) an owner of a file
        [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_client_cdn.json
        if [ -f "/root/easy-xray-main/conf/config_client_cdn.json" ]
        then
...
# the main part of `/root/easy-xray-main/ex.sh add` command, adds config for given users and updates server config
add () {
    export PATH=$PATH:/usr/local/bin/ # brings xray to the path for sudo user
    check_command xray "needed for config generation" "to install xray, try: sudo /root/easy-xray-main/ex.sh install"
    check_command jq "needed for operations with configs"
...
but not present; to generate them, try
    /root/easy-xray-main/ex.sh conf${normal}"
        exit 1
    fi
    if $1
    then
...
preferably of letters and digits only."
    ...
    cp_to_backup /root/easy-xray-main/conf/config_server.json
    # loop over usernames
    for username in "${@:2}"
    do
        username_exists=false
        client_emails=$(jq ".inbounds[1].settings.clients[].email" /root/easy-xray-main/conf/config_server.json)
...
no new config created fot it${normal}"
            ...
            if $resume
            then
                if [ ! -f "/root/easy-xray-main/conf/config_client_$username.json" ]
                then
                    echo -e "${red}no /root/easy-xray-main/conf/config_client_${username}.json found, can't resume${normal}"
                    exit 1
                fi
                id=$(strip_quotes $(jq ".outbounds[0].settings.vnext[0].users[0].id" /root/easy-xray-main/conf/config_client_${username}.json))
                short_id=$(strip_quotes $(jq ".outbounds[0].streamSettings.realitySettings.shortId" /root/easy-xray-main/conf/config_client_${username}.json))
                ok1=true
            else
                ...
                ok1=$(cat /root/easy-xray-main/conf/config_client.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\" | .outbounds[0].settings.vnext[0].users[0].email=\"${username}@example.com\" | .outbounds[0].streamSettings.realitySettings.shortId=\"${short_id}\"" > /root/easy-xray-main/conf/config_client_${username}.json)
                # then make the user (not root) an owner of a file
                [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_client_${username}.json
                if [ -f "/root/easy-xray-main/conf/config_client_cdn.json" ]
                then
                    cat /root/easy-xray-main/conf/config_client_cdn.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\"" > /root/easy-xray-main/conf/config_client_${username}_cdn.json
                [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_client_${username}_cdn.json
                fi
            fi
            ...
            grpc_client_id="
          {
            ...
            "
            cp /root/easy-xray-main/conf/config_server.json /root/easy-xray-main/conf/tmp_server_config.json
            ok2=$(cat /root/easy-xray-main/conf/tmp_server_config.json | jq ".inbounds[1].settings.clients += [${client}] | .inbounds[1].streamSettings.realitySettings.shortIds += [\"${short_id}\"] | .inbounds[3].settings.clients += [${grpc_client_id}]" > /root/easy-xray-main/conf/config_server.json)
            # then make the user (not root) an owner of a file
            [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_server.json
            if $ok1 && $ok2
            then
                rm /root/easy-xray-main/conf/tmp_server_config.json
                if [ ! $resume ]
                then
...
# `/root/easy-xray-main/ex.sh push` command, copies config to xray's dir and restarts xray
push () {
    if [ $(id -u) -ne 0 ] # not root
    then
        echo -e "${red}you should have root privileges for that, try
sudo /root/easy-xray-main/ex.sh push${normal}"
        exit 1
    fi
    ...
    #     echo -e "Which config from /root/easy-xray-main/conf/ to use? (write the filename)"
    #     read answer
    #     config="$answer"
    # else
        # use server config
    ...
    # fi
    if $(cp /root/easy-xray-main/conf/${config} /usr/local/etc/xray/config.json && systemctl restart xray)
    then
...
sudo xray run -c /root/easy-xray-main/conf/${config}${normal}"
    fi
}
...
sudo /root/easy-xray-main/ex.sh install${normal}"
        exit 1
    fi
    #
    ...
    then
        ...
        #
        cp -b /root/easy-xray-main/cert.pem /etc/ssl/certs/
        cp -b /root/easy-xray-main/cert.key /etc/ssl/private/
        cp -b /root/easy-xray-main/nginx.conf /etc/nginx/nginx.conf
    else
...
then
    ...
    check_command jq "needed for operations with configs"
    if [ ! -f "/root/easy-xray-main/conf/config_server.json" ]
    then
    ...
    for username in "${@:2}"
    do
        config="/root/easy-xray-main/conf/config_client_${username}.json"
        if [ ! -f $config ]
        then
            ...
            cp /root/easy-xray-main/conf/config_server.json /root/easy-xray-main/conf/tmp_server_config.json
            # update server config
            ok1=$(cat /root/easy-xray-main/conf/tmp_server_config.json | jq "del(.inbounds[1].settings.clients[] | select(.email == \"${username}@example.com\")) | del(.inbounds[1].streamSettings.realitySettings.shortIds[] | select(. == ${short_id})) | del(.inbounds[3].settings.clients[] | select(.id == ${id}))" > /root/easy-xray-main/conf/config_server.json)
            # then make the user (not root) an owner of a file
            [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_server.json
            if [ $command = "del" ]
            then
                ok2=$(rm /root/easy-xray-main/conf/config_client_${username}.json)
                if [ -f "/root/easy-xray-main/conf/config_client_${username}_cdn.json" ]
                then
                    rm /root/easy-xray-main/conf/config_client_${username}_cdn.json
                fi
                if $ok1 && $ok2
                then
                    rm /root/easy-xray-main/conf/tmp_server_config.json
...
then
    ...
    then
        ...
        #
        # Per user statistics
        conf_file="/root/easy-xray-main/conf/config_server.json" # assuming xray is running with this config
...
then
    ...
    continue with name from email${normal}"
            ...
            then
                ...
                if [ -f "/root/easy-xray-main/conf/config_client_cdn.json" ]
                then
                    cat /root/easy-xray-main/conf/config_client_cdn.json | jq ".outbounds[0].settings.vnext[0].users[0].id=\"${id}\"" > /root/easy-xray-main/conf/config_client_${uname_from_email}_cdn.json
                [[ $SUDO_USER ]] && chown "$SUDO_USER:$SUDO_USER" /root/easy-xray-main/conf/config_client_${username}_cdn.json
                fi
                # update server config
                client="
...
configs for other users ${italic}config_client_*.json${normal}. How to use it:
    ...
    ${bold}/root/easy-xray-main/ex.sh ${underl}command${normal}
